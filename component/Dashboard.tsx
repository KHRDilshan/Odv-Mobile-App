...
//       datasets: [
//         { data: [...prev.datasets[0].data, value].slice(-MAX_POINTS) },
//       ],
//     });
...
//               <Text className="text-white   -mt-2 text-base font-semibold">
//                 Temperature Local
//               </Text>
           
// <View className="flex-row">
//   {[
//     { color: "#dd9e36", active: state === "ENDTREATMENT" }, // yellow
//     { color: "#10b981", active: state === "TREATMENT" },    // green
//     { color: "#ef4444", active: state === "HEATING" },      // red
//     { color: "#3b82f6", active: state === "INIT" },         // blue
//     { color: "#8b5cf6", active: state === "REINIT" },       // purple
//   ].map((item, index) => (
//     <View
//       key={index}
//       style={{
...
//         name="lightbulb"
//         size={18}
//         color={item.active ? item.color : "transparent"}
//         style={{
...
//             >
//               <Text className="text-sm text-gray-400 font-semibold mb-1">
//                 Temperature Local
//               </Text>
...
import { MaterialIcons, Foundation, FontAwesome5, FontAwesome6 } from "@expo/vector-icons";
import { LineChart } from "react-native-chart-kit";
import Svg, {
  Path,
  G,
...
// Ensure chart includes -5 in the scale
const prepareChartData = (data: ChartDataType) => ({
  labels: data.labels,
  datasets: [
    {
      data: data.datasets[0].data.some(v => v < 0)
        ? [-5, ...data.datasets[0].data] // force min -5
...
<Text className="text-white font-semibold text-base ">State</Text>
   <Text
  className={`
    ${state === "ENDTREATMENT" ? "text-yellow-400" : ""}
    ${state === "TREATMENT" ? "text-emerald-500" : ""}
    ${state === "HEATING" ? "text-red-500" : ""}
    ${state === "INIT" ? "text-blue-500" : ""}
    ${state === "REINIT" ? "text-violet-500" : ""}
    font-semibold
  `}
>
  {state}
  ...
  {[
       ...
       { color: "#3b82f6", active: state === "INIT" }, 
       { color: "#8b5cf6", active: state === "REINIT" }, 
    { color: "#dd9e36", active: state === "ENDTREATMENT" },
    { color: "#10b981", active: state === "TREATMENT" },   
    { color: "#ef4444", active: state === "HEATING" },     
  ].map((item, index) => (
    <View
      key={index}
      style={{
...
<View className="flex-row justify-between -mb-10 z-50  px-10 ">
  ...
  chartConfig={{
    ...
    propsForDots: { r: "3", strokeWidth: "2", stroke: item.color },
        propsForLabels: {
      fontSize: 14, // default is 12, increase as needed
    },
