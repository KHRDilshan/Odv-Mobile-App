...
export default function AnalyzeScreen() {
  ...
  const timeToSeconds = (t: { h: string; m: string; s: string }) =>
    parseInt(t.h) * 3600 + parseInt(t.m) * 60 + parseInt(t.s);
  ...
  const getSensorData = async () => {
    ...
    if (endSec - startSec > 300) {
      Alert.alert("Time range cannot exceed 5 minutes (300 seconds)");
      return;
    }
    ...
    try {
      const query = `?startDate=${selectedDate}&startTime=${startTime}&endDate=${selectedDate}&endTime=${endTime}&parameter=${parameter}`;
...
const chartData = {
  labels: filteredData.map(item => {
    ...
    const cleanTime = item.time.split("T")[1]?.split(".")[0]; // "09:27:39"
    ...
    <LinearGradient colors={["#040e16", "#061420"]} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} className="flex-1">
      ...
      <ScrollView className="px-4">
        ...
        <View className="bg-[#071b2a] border border-[#0b2a3c] rounded-2xl p-4 mb-4 ">
          <Text className="text-white text-lg font-semibold text-center mb-2">Select Date & Time</Text>
                      <DateBox className="" date={date} onPress={() => setShowDate(true)} />
          ...
          <View className="flex-row items-center justify-center mb-4">
            ...
            <Text className="text-white mx-2 font-bold">→</Text>
        ...
        >
          ...
          <Text style={{ textAlign: "center", color: "#94a3b8", marginTop: 8, fontWeight: "500" }}>
            Sensor History
          </Text>
        </View>
      </ScrollView>
      ...
      {showDate && (
        <DateTimePicker
          value={date}
          mode="date"
          onChange={(_, d) => d && (setDate(d), setShowDate(false))}
        />
      )}
